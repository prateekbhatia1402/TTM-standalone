...
public class TTCreation extends javax.swing.JFrame {
    ...
    private LocalDate wef = LocalDate.now().plusDays(1);
    ...
    ArrayList<ArrayList<Color>> ttColorData = new ArrayList<>();
    private TableCellRenderer ttCellRenderer;
    ArrayList<ArrayList<Color>> subColorData = new ArrayList<>();
    private TableCellRenderer subCellRenderer;
    ...
    private void initTables(String tid) {
        ...
        for (int i = 0; i < days.length; i++)
        {
            ...
            ttColorData.add(colors);
        }
        var columnModel = jTimeTable.getColumnModel();
        ttCellRenderer = new CellColorRenderer(ttColorData);
        for (int i = 1; i<= periods.length; i++)
            columnModel.getColumn(i).setCellRenderer(ttCellRenderer);
        ...
        if (v == null || v.isEmpty()) {
            ...
            jChangeClassButtonActionPerformed(null);
            return;
        }
        ...
        allFacSubDetailsList.forEach(facSub -> {
            System.out.println(facSub.toString());
        ...
        while (wef == null)
        {
           ...
           if (date.trim().toLowerCase().startsWith("n"))
           {
            ...
            wef = from;
                ...
                for (int i = 0; i < allFacSubDetailsList.size(); i++)
        {
            ArrayList<Color> colors = new ArrayList<>();
            colors.add(Color.decode(allFacSubDetailsList.get(i).getColor()));
            colors.add(Color.white);
            colors.add(Color.white);
            subColorData.add(colors);
        }
        var subColumnModel = jSubjectAllotedTable.getColumnModel();
        subCellRenderer = new CellColorRenderer(subColorData);
        for (int i = 0; i< 3; i++)
            subColumnModel.getColumn(i).setCellRenderer(subCellRenderer);
    ...
    private void clearTable() {
        ...
        {
            facSub.resetLect();
            subColorData.get(count).set(1, Color.red);
            subColorData.get(count).set(2, Color.red);
            subjectTableModel.addRow(
                    new Object[]{facSub.getSubName(), facSub.getLectReq(), 0});
            count++;
        }
        for (int i = 0; i< days.length; i++)
            for (int j = 0; j<= periods.length; j++)
                ttColorData.get(i).set(j, Color.white);
    }
    ...
    private void initTableData(ArrayList<Schedule> schedule) {
        ...
        for (Schedule record : schedule) {
            ...
            String subjectName = null, subjectId = record.getSubjectId();
            String facName = "";
            var facSub = allFacSubDetailsMap.get(subjectId);
            if(!facSub.isFree(record.getDayId(), record.getTimeslotId()))
                continue;
            String fac_sub = facSub.getFacId() + "_" + subjectId;
            facName = facSub.getFacName();
            subjectName = facSub.getSubName();
            timeTable[day][period] = fac_sub;
            increaseSubCount(subjectId);
            ttColorData.get(day).set(period+1, Color.decode(facSub.getColor()));
            timeTableModel.setValueAt(facName + " " + subjectName,
                             day, period + 1);
        }
    }
    ...
    TTCreation(TimeTables timeTables, String tid) {
        ...
        jFastSelectButton.setEnabled(false);
        classes = TimeTableControl.getClasses();
        jChangeClassButton.setEnabled(true);
        jWefTextField.setText(LocalDate.now().plusDays(1).format(DateTimeFormatter.ofPattern("d-MM-yyyy")));
        jClassBox.removeAllItems();
        this.timeTables = timeTables;
        selectedClass = ClassControl.getClass(timeTables.tablesOf);
        jClassBox.addItem("select class");
        String svalue = "";
        for (I_Class classe : classes) {
            ...
            if (timeTables.tablesOf.equals(classe.getId())) {
                svalue = value;
            }
        }
        jClassSelectButton.setEnabled(false);
    ...
    public void sugestSchedulev3() {
        ...
        facSubToAssign.forEach(facSub -> {
            System.out.println(facSub.getSubName()
    ...
    private void updateCell(int day, int period, String subId) {
        if (subId.equalsIgnoreCase("free")) {
            ...
            timeTableModel.setValueAt("FREE", day, period + 1);
                ttColorData.get(day).set(period + 1, Color.white);
            return;
        }
    ...
    private void updateCell(int day, int period, FacSub facSub) {
        ...
        System.out.println("updating cell "+day+" "+period+" "
                + facSub.getFacId() + "_" + facSub.getSubId());
        increaseSubCount(facSub.getSubId());
        String val = facSub.getFacName() + " " + facSub.getSubName();
        
                ttColorData.get(day).set(period + 1, Color.decode(facSub.getColor()));
        timeTableModel.setValueAt(val,
                day, period + 1);
    }
    ...
    private void updateList() {
        ...
        for (var v : allFacSubDetailsList) {
            listColorData.putIfAbsent(v.getFacName() + "(" + v.getFacId() + ") : "
                    + v.getSubName(), Color.decode(v.getColor()));
    ...
    private void initComponents() {
        ...
        jAutomateButton = new javax.swing.JButton();
        ...
        jWefTextField = new javax.swing.JTextField();
        jChangeClassButton = new javax.swing.JButton();
        ...
        jPanel1Layout.setVerticalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGap(18, 18, 18)
                .addComponent(jFastSelectButton, javax.swing.GroupLayout.DEFAULT_SIZE, 85, Short.MAX_VALUE)
                .addGap(65, 65, 65))
        );
        ...
        jSubjectAllotedTable.setAutoResizeMode(javax.swing.JTable.AUTO_RESIZE_ALL_COLUMNS);
        ...
        jAutomateButton.setText("automate");
        jAutomateButton.setEnabled(false);
        jAutomateButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jAutomateButtonActionPerformed(evt);
            }
        });

        jClearAllButton.setText("CLEAR ALL");
        jClearAllButton.setEnabled(false);
        ...
        jWefTextField.setToolTipText("wef");
        jWefTextField.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jWefTextFieldActionPerformed(evt);
            }
        });
        jWefTextField.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                jWefTextFieldKeyTyped(evt);
            }
        });

        jChangeClassButton.setText("Change");
        jChangeClassButton.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jChangeClassButtonActionPerformed(evt);
            }
        });
        ...
        jPanel4Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel4Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jWefTextField, javax.swing.GroupLayout.PREFERRED_SIZE, 115, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jChangeClassButton)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                ...
                .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel4Layout.createSequentialGroup()
                        .addGroup(jPanel4Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            ...
                            .addComponent(jWefTextField, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(jChangeClassButton))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)))
                .addComponent(jClassSelectButton))
            .addGroup(jPanel4Layout.createSequentialGroup()
    ...
    private void setComponents(boolean isClassSelected)
    {
        jClassBox.setEnabled(!isClassSelected);
        jClassSelectButton.setEnabled(!isClassSelected);
        jWefTextField.setEnabled(!isClassSelected);
        jChangeClassButton.setEnabled(isClassSelected);
        jClearSelectionButton.setEnabled(isClassSelected);
        jClearAllButton.setEnabled(isClassSelected);
        jFastSelectButton.setEnabled(isClassSelected);
        jAutomateButton.setEnabled(isClassSelected);
        jSubmitButton.setEnabled(isClassSelected);
        jLoadButton.setEnabled(isClassSelected);
    }
    ...
    private void jClassBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jClassBoxActionPerformed
        toggleClassSelectionButton();
    }//GEN-LAST:event_jClassBoxActionPerformed

    private void toggleClassSelectionButton() {
        if (jClassBox.getSelectedIndex() > 0 && 
           jWefTextField.getText().length() >= 8) {
    ...
    private void checkConstraints() {
        ...
        for (var facSub : allFacSubDetailsList) {
            ...
            subColorData.get(count).set(0, Color.decode(facSub.getColor()));
            subColorData.get(count).set(1, Color.decode(color));
            subColorData.get(count).set(2, Color.decode(color));
            subjectTableModel.addRow(
                    new Object[]{facSub.getSubName(),facSub.getLectReq(),
                       facSub.getLectAssigned()
                    }
            );
            count++;
    ...
    private void jClassSelectButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jClassSelectButtonActionPerformed
        ...
        System.out.println(timeTables.isUpdateScheduled());
        
        try {
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("d-MM-yyyy");
            LocalDate wef = LocalDate.parse(jWefTextField.getText(), formatter);
            LocalDate today = LocalDate.now();
            if (!wef.isAfter(today)) {
                JOptionPane.showMessageDialog(this, "kindly choose wef such that  wef is after today",
                        "Invalid Date", JOptionPane.ERROR_MESSAGE);
                return;
            }
            this.wef = wef;
        } catch (Exception e) {
            JOptionPane.showMessageDialog(this, "Invalid Date Format\nkindly rectify the error in wef date",
                    "Invalid Date", JOptionPane.ERROR_MESSAGE);
           System.err.println(e.getMessage()+"\n");
           Logger.getLogger(TTCreation.class.getName()).log(Level.SEVERE, null, e);
            return;
        }
        if (timeTables.isUpdateScheduled()) {
            int res = JOptionPane.showConfirmDialog(this, "this class already has a time table "
                    + "change scheduled.\n"
                    + "if you choose to update schedule anyway, the current scheduled update will be discarded and deleted.\n"
                    + "Are you sure you want to continue?\n\n"
                    + "P.S. this message may also appear while you try to save the changes",
                    "Change Already Scheduled", JOptionPane.YES_NO_CANCEL_OPTION,
        ...
        jClassSelectButton.setEnabled(false);
        jClassBox.setEnabled(false);
        jWefTextField.setEnabled(false);
        jChangeClassButton.setEnabled(true);
        jFastSelectButton.setEnabled(true);
        jClearAllButton.setEnabled(true);
        jClearSelectionButton.setEnabled(true);
        jAutomateButton.setEnabled(true);
        jLoadButton.setEnabled(true);
        setComponents(true);
        initTables(defaultTid);
    ...
    private void jSubmitButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jSubmitButtonActionPerformed
        ...
        int res = TimeTableControl.createTimeTable(selectedClass.getId(),
                schedule, wef);
    ...
    private void jAutomateButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jAutomateButtonActionPerformed
        sugestSchedulev3();
    }//GEN-LAST:event_jAutomateButtonActionPerformed
    ...
    private void jWefTextFieldActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jWefTextFieldActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_jWefTextFieldActionPerformed

    private void jWefTextFieldKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jWefTextFieldKeyTyped
       toggleClassSelectionButton();
    }//GEN-LAST:event_jWefTextFieldKeyTyped

    private void jChangeClassButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jChangeClassButtonActionPerformed
    resetForm();
    }//GEN-LAST:event_jChangeClassButtonActionPerformed
    ...
    void applyFastSelection() {
        ...
        for (int i = 0; i < days.length; i++) {
            for (int j = 0; j < periods.length; j++) {
                ...
                var id = cell.id;
                System.out.println(i+" "+j+" "+cell.id);
    ...
    private javax.swing.JButton jAutomateButton;
    private javax.swing.JButton jButton5;
    private javax.swing.JButton jChangeClassButton;
    ...
    private javax.swing.JTextField jWefTextField;
